...
const CONFIG = {
  ...
  // خصم عند محاولة أخذ تذكرة محجوزة
...
function getRank(member) {
  ...
  const developerRole = getDeveloperRole(member);
...
async function getOrCreateRankRole(guild, rankIndex) {
...
async function checkPromotion(member) {
  ...
  // غير المطور لا يدخل نظام النقاط
  ...
  // الإداري بدون رتبة مطور لا تتم ترقيته
  ...
  if (
    newRankIndex <= currentRole.index
  ) {
...
client.once(
  ...
  async () => {
    ...
    // تنظيف نقاط الأشخاص الذين ليس لديهم رتبة مطور
  ...
  async message => {
    ...
    if (
      ...
      if (
        ...
        return message.reply(
          "❌ أنت لست ضمن نظام المطورين، لذلك لا يوجد لديك نظام نقاط."
        );
      ...
      if (!rank) {
        ...
        return message.reply(
          "❌ أنت إداري ولكن لا تملك رتبة مطور."
        );
    ...
    if (
      ...
      if (!newRole) {
        ...
        return message.reply(
          "❌ لم أستطع إنشاء رتبة المطور."
        );

      }

      // إزالة رتب المطور القديمة
      for (
        const rank
        of CONFIG.RANKS
      ) {
      ...
      try {
        ...
        return message.reply(
          "❌ لم أستطع إعطاء الرتبة. تأكد أن رتبة البوت أعلى من رتب المطورين."
        );

      }

      // يبدأ من صفر عند تعيين رتبة جديدة
      setPoints(
        user.id,
        0
      );
    ...
    if (
      ...
      if (
        ...
        return message.reply(
          "❌ ليس لديك رتبة مطور، لذلك لا يوجد لك راتب."
        );
      ...
      await sendLog(
        ...
        `${message.author} استلم راتب **${rank.rank.salary}**`
